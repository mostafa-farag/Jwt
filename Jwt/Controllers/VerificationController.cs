...
public class VerificationController : ControllerBase
{
    ...
    {
        var lastData = _context.VerificationCheckRequest
                .Where(item => item.VerificationCode == code)
            //    .OrderByDescending(item => item.Email) // Replace DateColumn with your actual column
                .Select(item => item.VerificationCode) // Replace YourColumn with the desired column
                .FirstOrDefault();

        //   var lastVerificationCode = _context.VerificationCheckRequest
        //.OrderByDescending(vc => vc.Email).Select(vc => vc.VerificationCode).FirstOrDefault();
        if (code != lastData)
        {
            return BadRequest("Your code is False");
        }
        return Ok("your code is true");
    }
